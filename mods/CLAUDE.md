...
- **`README.md`/`CHANGELOG.md`/`manifest.json` de chaque mod sont des documents PUBLICS**,
  livrés dans le zip distribué (Thunderstore/Hexium...) et lus par n'importe quel joueur
  Valheim dans le monde -- pas seulement la communauté Fedoheim, et surtout pas seulement
  quelqu'un qui a accès à ce repo. Ne jamais y écrire : une référence à "ce repo"/ce
  projet, à l'API ou au launcher Fedoheim, à un autre mod maison **non publié** (même en
  comparaison, ex. "comme le fait tel autre mod interne"), ou tout détail
  d'implémentation (nom de champ patché, technique de reflection, provenance d'une
  décompilation...). Un lien vers un AUTRE mod déjà publié (ex. FedoHud) reste ok si
  l'info est réellement utile au lecteur. Avant de finaliser ces fichiers, se relire en
  se mettant à la place d'un inconnu qui n'a jamais vu ce repo ni entendu parler d'un
  autre mod de cette communauté. Les notes techniques vont dans ce fichier
  (`mods/CLAUDE.md`), jamais dans les docs livrées avec le mod.
...
- Pour inspecter les signatures/accessibilité exactes d'`assembly_valheim.dll` avant d'écrire un patch Harmony (éviter de deviner un nom de champ/méthode) : petit programme jetable utilisant `System.Reflection.MetadataLoadContext` (package NuGet) pointé sur le dossier `Managed` du jeu -- plus fiable que de se fier à la mémoire/documentation communautaire. Cette technique donne les signatures (champs/méthodes/accessibilité) mais jamais le CORPS d'une méthode (ex. quel enfant de quel nom un `UI.Setup` va chercher) -- pour ça, `ilspycmd` (déjà installé sur cette machine via `dotnet tool`, invoqué avec `DOTNET_ROLL_FORWARD=LatestMajor` pour tourner sur le SDK .NET présent) décompile une classe entière en C# lisible à partir du même `assembly_valheim.dll` : `ilspycmd -t NomDeLaClasse assembly_valheim.dll`. Utilisé pour trouver la structure exacte de `SkillsDialog.Setup` (voir `FedoSkillSafety/SkillsDialogPatch.cs`) : chaque ligne du panneau de compétences (Tab) est un enfant nommé (`Utils.FindChild(elementTransform, "leveltext")`, etc.) plutôt qu'un champ dédié.
- **Dépendance douce vers un AUTRE mod maison de ce repo** (pas Seasons/assembly_valheim,
  un mod dont on n'a pas le contrôle du côté qui dépend) : si le type visé est `internal`
  côté mod cible (ex. `FedoHud.SkillsOverlay`, voir `FedoSkillSafety/FedoHudIntegration.cs`),
  ajouter une référence de projet ne suffit pas -- il reste inaccessible au compilateur
  même référencé. Tout doit passer par réflexion pure (`AccessTools.TypeByName`/
  `AccessTools.Method`/`AccessTools.Inner`, patch posé à la main via `harmony.Patch(...)`
  plutôt que l'attribut `[HarmonyPatch(typeof(...))]`, qui exige un type résolu à la
